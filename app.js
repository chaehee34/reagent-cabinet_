(function () {
  ...
  async function loadMemos(items) {
    for (const item of items) {
      const input = els.tableBody.querySelector(`[data-memo-id="${item.id}"]`);
      if (!input) continue;
      const note = await window.NoteStorage.getNote(item.id);
      input.value = note || '';
      input.disabled = false;
      bindMemoInput(input, item.id);
    }
  }

  function bindMemoInput(input, id) {
    let lastSaved = input.value;

    const save = async () => {
      const text = input.value;
      if (text === lastSaved) return;
      input.classList.add('memo-saving');
      const ok = await window.NoteStorage.setNote(id, text);
      input.classList.remove('memo-saving');
      if (ok) {
        lastSaved = text;
        input.classList.remove('memo-error');
      } else {
        input.classList.add('memo-error');
      }
    };
