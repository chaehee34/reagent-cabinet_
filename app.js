(function () {
  ...
  const els = {
    ...
    detailMw: document.getElementById('detail-mw'),
    detailPurity: document.getElementById('detail-purity'),
    detailManuf: document.getElementById('detail-manuf'),
  ...
  function bindEvents() {
    ...
    els.memoSave.addEventListener('click', saveMemo);
  }
  ...
  async function render() {
    ...
    for (const item of pageItems) {
      ...
      tr.innerHTML = `
        ...
        <td class="col-memo"><span class="memo-dot" data-memo-dot="${item.id}"></span></td>
      `;
    ...
    refreshMemoDots(pageItems);
  }

  async function refreshMemoDots(items) {
    for (const item of items) {
      const has = await window.NoteStorage.hasNote(item.id);
      const dot = els.tableBody.querySelector(`[data-memo-dot="${item.id}"]`);
      if (dot) dot.classList.toggle('filled', has);
    }
  }
